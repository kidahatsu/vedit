    ...
    const handleReverse = useCallback(
        async (id: string) => {
            ...
            if (duration > 10) {
                if (
                    !confirm(
                        `This clip is ${Math.round(duration)}s long. Reversing long clips (>10s) may consume significant memory. Proceed?`
                    )
                ) {
                    return
                }
    ...
    return (
        <aside className={styles.panel} aria-label="Media asset drawer">
            <div className={styles.panelHeader}>
                ...
                {clips.length > 0 && <span className={styles.countBadge}>{clips.length}</span>}
            </div>
            ...
            {clips.length === 0 ? (
                ...
                >
                    ...
                    <div className={styles.dropSubtitle}>Drop MP4/MOV or click to browse</div>
                </div>
            ) : (
                <>
                    <div
