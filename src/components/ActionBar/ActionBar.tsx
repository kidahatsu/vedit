...
export function ActionBar({ onOpenExportModal }: ActionBarProps) {
    ...
    const handleReverse = async () => {
        ...
        if (duration > 10) {
            if (
                !confirm(
                    `This clip is ${Math.round(duration)}s long. Reversing long clips (>10s) may consume significant memory and crash the application. Do you want to proceed?`
                )
            ) {
                return
            }
        ...
        try {
            ...
            const clipStillExists = useEditorStore
                .getState()
                .clips.some((c) => c.id === activeClipId)
            ...
            alert(
                'Failed to reverse clip. The browser might have run out of memory. Try using a shorter clip or lower resolution.'
            )
        } finally {
            setLoading(false)
        }
    ...
    const handleExtractFrame = async (mode: 'first' | 'last') => {
        ...
        try {
            ...
            const frameBlob = await extractFrame(selectedClip.file, time, (progress, message) =>
                ...
                <div className={styles.leftCluster}>
                    <div className={styles.group}>
                        ...
                        >
                            ...
                            <span>
                                {canSplit
                                    ? `Split (${selectedClip!.splitPoints.length + 1})`
                                    : 'Export Split'}
                            </span>
                        </button>
