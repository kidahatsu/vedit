...
import { Scissors, Split, Link, RotateCcw, Image, Film } from 'lucide-react'
import { useEditorStore } from '../../store/editorStore'
import {
    useSelectedClip,
    useCanMerge,
    useCanSplit,
    useSelectedClipHasModifications,
...
export function Timeline({ onOpenExportModal }: TimelineProps) {
    ...
    const throttledSeekPreview = useCallback(
        (time: number) => {
            const now = Date.now()
    ...
    const handleSplitMarkerDragStart = useCallback(
        (e: React.MouseEvent, splitTime: number) => {
            e.stopPropagation()
            e.preventDefault()
            if (!selectedClip || !trackRef.current) return
            ...
            const handleMouseMove = (moveEvent: MouseEvent) => {
                ...
                const percent = clamp(x / trackRect.width, 0, 1)
                const time = percent * duration
                lastClampedTime = clamp(
                    time,
                    selectedClip.trimStart + 0.01,
                    selectedClip.trimEnd - 0.01
                )
        ...
        onOpenExportModal('trim', async (preset: ExportPreset) => {
            try {
                ...
                const needsTransform =
                    hasTransformsApplied(selectedClip.transform) ||
                ...
                if (needsTransform) {
                    ...
                    if (gpuSupported && transform.speed === 1 && aspectRatio === 'original') {
                        blob = await exportVideoWithWebGPU({
                            ...
                            width:
                                preset.resolution.width > 0 ? preset.resolution.width : undefined,
                            height:
                                preset.resolution.height > 0 ? preset.resolution.height : undefined,
    ...
    const effectiveTrimStart =
        selectedClip && draggingTrim?.handle === 'left'
            ? draggingTrim.time
            : (selectedClip?.trimStart ?? 0)
    const effectiveTrimEnd =
        selectedClip && draggingTrim?.handle === 'right'
            ? draggingTrim.time
            : (selectedClip?.trimEnd ?? 1)
        ...
        <div className={styles.container} role="region" aria-label="Magnetic Studio Timeline">
            ...
            <div className={styles.toolbar}>
                <div className={styles.toolGroup}>
                    <button
                        ...
                        title={
                            splitMode
                                ? 'Exit Split Mode'
                                : 'Enter Split Mode (Click timeline to add cuts)'
                        }
                    >
                        <Split size={13} />
                    ...
                    <button
                        ...
                        title={
                            canSplit
                                ? `Export ${selectedClip!.splitPoints.length + 1} Segments`
                                : 'Add split points in timeline first'
                        }
                    >
                        <span>
                            {canSplit
                                ? `Export Split (${selectedClip!.splitPoints.length + 1})`
                                : 'Export Split'}
                        </span>
                    </button>
                ...
                {selectedClip && (
                    <div className={styles.timecodeGroup}>
                        ...
                        <span className={styles.timecodeValue}>
                        ...
                        <span className={styles.timecodeValue}>
                            {formatTime(selectedClip.trimEnd - selectedClip.trimStart)}
                        </span>
                    </div>
                )}
            </div>
            ...
            <div className={styles.ruler}>
                {rulerTicks.map((tick, i) => (
                    ...
                    >
                        {i % 2 === 0 && <span className={styles.rulerLabel}>{tick.label}</span>}
                    </div>
                ))}
            </div>
                ...
                {selectedClip ? (
                    ...
                    >
                        ...
                        <div
                            ...
                            onKeyDown={(e) => {
                                ...
                                if (e.key === 'ArrowLeft') {
                                    ...
                                    updateClipTrim(
                                        selectedClip.id,
                                        Math.max(0, selectedClip.trimStart - step),
                                        selectedClip.trimEnd
                                    )
                                    ...
                                    updateClipTrim(
                                        selectedClip.id,
                                        Math.min(
                                            selectedClip.trimEnd - 0.1,
                                            selectedClip.trimStart + step
                                        ),
                                        selectedClip.trimEnd
                                    )
                                }
                            ...
                            onKeyDown={(e) => {
                                ...
                                if (e.key === 'ArrowLeft') {
                                    ...
                                    updateClipTrim(
                                        selectedClip.id,
                                        selectedClip.trimStart,
                                        Math.min(selectedClip.duration, selectedClip.trimEnd + step)
                                    )
                                }
                            }}
                            role="slider"
                        ...
                        {selectedClip.splitPoints.map((splitTime) => {
                            ...
                            const displayTime =
                                isDragging && dragPreviewTime !== null ? dragPreviewTime : splitTime
                                    ...
                                    onKeyDown={(e) => {
                                        if (e.key === 'Delete' || e.key === 'Backspace') {
                                            ...
                                            const newTime = Math.max(
                                                selectedClip.trimStart + 0.05,
                                                splitTime - (e.shiftKey ? 0.5 : 0.05)
                                            )
                                            updateSplitPoint(
                                                selectedClip.id,
                                                splitTime,
                                                Number(newTime.toFixed(3))
                                            )
                                        } else if (e.key === 'ArrowRight') {
                                            e.preventDefault()
                                            const newTime = Math.min(
                                                selectedClip.trimEnd - 0.05,
                                                splitTime + (e.shiftKey ? 0.5 : 0.05)
                                            )
                                            updateSplitPoint(
                                                selectedClip.id,
