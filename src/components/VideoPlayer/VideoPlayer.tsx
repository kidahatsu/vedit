...
export function VideoPlayer() {
    ...
    useEffect(() => {
        ...
        if (
            blobUrlRef.current?.clipId === currentClipId &&
            blobUrlRef.current?.file === currentFile
        ) {
        ...
        (e: MouseEvent) => {
            ...
            switch (dragHandle) {
                ...
                case 'topLeft':
                    newX = Math.max(
                        0,
                        Math.min(cropStart.x + cropStart.width - 0.05, cropStart.x + deltaX)
                    )
                    newY = Math.max(
                        0,
                        Math.min(cropStart.y + cropStart.height - 0.05, cropStart.y + deltaY)
                    )
    ...
    const aspectRatioLabel =
        selectedClip?.transform.aspectRatio !== 'original'
            ? selectedClip?.transform.aspectRatio
            : null

    const showCropOverlay = Boolean(
        (cropMode ||
            isCropped ||
            (selectedClip && selectedClip.transform.aspectRatio !== 'original')) &&
        selectedClip &&
        cropBox
    )
                                ...
                                <div className={styles.cropOverlay}>
                                    ...
                                    >
                                        ...
                                        <div className={styles.cropGrid}>
                                            <div
                                                className={`${styles.cropGridLine} ${styles.h1}`}
                                            />
                                            <div
                                                className={`${styles.cropGridLine} ${styles.h2}`}
                                            />
                                            <div
                                                className={`${styles.cropGridLine} ${styles.v1}`}
                                            />
                                            <div
                                                className={`${styles.cropGridLine} ${styles.v2}`}
                                            />
                                        </div>
                                        ...
                                        <div
                                            ...
                                            onMouseDown={(e) =>
                                                handleCropMouseDown(e, 'bottomLeft')
                                            }
                                        />
                                        <div
                                            className={`${styles.cropHandle} ${styles.bottomRight}`}
                                            onMouseDown={(e) =>
                                                handleCropMouseDown(e, 'bottomRight')
                                            }
                                        />
                                        <div
                                        ...
                                        <div className={styles.cropInfo}>
                                            {selectedClip.transform.aspectRatio !== 'original'
                                                ? `${selectedClip.transform.aspectRatio} `
                                                : ''}
                                            ({Math.round(selectedClip.transform.cropWidth * 100)}% ×{' '}
                                            {Math.round(selectedClip.transform.cropHeight * 100)}%)
                                        </div>
                                    </div>
                                </div>
                            )}
                        ...
                        {(codec?.includes('hevc') || codec?.includes('hvc1')) && (
                            <div className={styles.codecWarning}>
                                <div className={styles.codecWarningContent}>
                                    ...
                                    <p>
                                        This video uses the <strong>{codec}</strong> codec, which
                                        your browser doesn't support for preview.
                                    </p>
                                    <p className={styles.hint}>
                                        Use <strong>"Fix Visibility"</strong> in Actions to make it
                                        visible.
                                    </p>
                                </div>
                            </div>
