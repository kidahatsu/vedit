...
export async function exportVideoWithWebGPU(options: GPUExportOptions): Promise<Blob> {
    ...
    const fileObj =
        file instanceof File ? file : new File([file], 'input.mp4', { type: 'video/mp4' })
    ...
    try {
        ...
        const muxer = new Muxer({
            ...
            audio:
                audioInfo && audioSamples.length > 0
                    ? {
                          codec: 'aac',
        ...
        if (audioInfo && audioSamples.length > 0) {
            for (const sample of audioSamples) {
                const timestampUs = Math.max(
                    0,
                    Math.round((sample.cts / audioInfo.timescale - startTime) * 1_000_000)
                )
                const durationUs = Math.max(
                    0,
                    Math.round((sample.duration / audioInfo.timescale) * 1_000_000)
                )
        ...
        debug(
            'GPUExport',
            `Streaming ${videoSamples.length} frames through WebGPU WGSL shaders (${outWidth}x${outHeight})`
        )
            ...
            output: (frame) => {
                try {
                    ...
                    if (processedFrames % 15 === 0) {
                        ...
                        onProgress?.(
                            pct,
                            `Processing WebGPU frames (${processedFrames}/${videoSamples.length})...`
                        )
                    }
                } catch (err) {
