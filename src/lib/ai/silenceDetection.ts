...
export async function detectSilenceSplitPoints(
    ...
    const { thresholdDb = -35, minSilenceDurationSec = 0.4, paddingSec = 0.05 } = options
    ...
    try {
        ...
        try {
            ...
            const fileObj =
                file instanceof File
                    ? file
                    : new File([file], 'input.mp4', { type: file.type || 'video/mp4' })
        ...
        debug(
            'SilenceDetection',
            `Detected ${uniqueSorted.length} split markers from silence analysis`
        )
        return uniqueSorted
    } finally {
