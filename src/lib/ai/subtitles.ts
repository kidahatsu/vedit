...
export async function generateSubtitlesWebGPU(
    ...
    try {
        ...
        onProgress?.(
            20,
            hasWebGPU
                ? 'Loading Whisper model (WebGPU accelerated)...'
                : 'Loading Whisper model (WASM fallback)...'
        )
            ...
            'onnx-community/whisper-tiny.en',
            {
                ...
                progress_callback: (info: unknown) => {
                    ...
                    if (progressInfo?.progress !== undefined) {
                        ...
                        onProgress?.(
                            pct,
                            `Downloading AI weights: ${Math.round(progressInfo.progress * 100)}%`
                        )
                    }
                },
            }
        ...
        try {
            ...
            try {
                ...
                warn(
                    'Subtitles',
                    'Direct audio decode failed; extracting audio track via FFmpeg...'
                )
                const { extractAudio } = await import('../ffmpeg')
                const fileObj =
                    audioBlob instanceof File
                        ? audioBlob
                        : new File([audioBlob], 'input.mp4', {
                              type: audioBlob.type || 'video/mp4',
                          })
