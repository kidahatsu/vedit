...
export async function probeVideoMediabunny(file: File | Blob): Promise<MediabunnyVideoInfo> {
    ...
    try {
        ...
        debug(
            'Mediabunny',
            `Probed ${file.size} bytes: ${width}x${height}, duration=${duration.toFixed(2)}s, codec=${videoCodec}`
        )
...
export async function trimVideoMediabunny(
    ...
    try {
        ...
        debug(
            'Mediabunny',
            `Trim complete: output size = ${(buffer.byteLength / 1024 / 1024).toFixed(2)} MB`
        )
