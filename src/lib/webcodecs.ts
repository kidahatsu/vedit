 ...
 */
export async function demuxMP4(
    file: File,
    ...
    return new Promise((resolve, reject) => {
        ...
        const timer = setTimeout(() => {
            if (!settled) {
                ...
                reject(
                    new Error(
                        'MP4 demuxing timed out. The file may be corrupt or missing a valid MOOV atom.'
                    )
                )
            }
        }, timeoutMs)
            ...
            if (videoTrack) {
                ...
                demuxResult.video = {
                    ...
                    codecConfig: {
                        ...
                        description: getAVCDecoderConfig(
                            mp4File,
                            videoTrack.id,
                            demuxResult.buffer
                        ),
            ...
            if (audioTrack) {
                ...
                demuxResult.audio = {
                    ...
                    codecConfig: {
                        ...
                        description: getAACDecoderConfig(
                            mp4File,
                            audioTrack.id,
                            demuxResult.buffer
                        ),
...
function getAVCDecoderConfig(
    ...
    try {
        ...
        const entries = (stbl.stsd.entries ?? []) as unknown as Array<{
            format: string
...
function getAACDecoderConfig(
    ...
    try {
        ...
        const entries = (stbl.stsd.entries ?? []) as unknown as Array<{
            ...
            esds?: {
                esd?: {
                    descs?: Array<{ tag: number; descs?: Array<{ tag: number; data?: number[] }> }>
                }
            }
        }>
...
export async function splitVideoWebCodecs(
    ...
    for (let i = 0; i < segments.length; i++) {
        ...
        onProgress?.(
            baseProgress + progressPerSegment * 0.2,
            `Extracting frames for segment ${i + 1}...`
        )
        const videoSamples = await extractVideoSamples(
            mp4File,
            demuxResult.buffer,
            videoInfo,
            start,
        ...
        if (videoSamples.length === 0) {
            throw new Error(
                `No video samples found for segment ${i + 1} (${start.toFixed(2)}s - ${end.toFixed(2)}s)`
            )
        }

        let audioSamples: Sample[] = []
        if (audioInfo) {
            audioSamples = await extractAudioSamples(
                mp4File,
                demuxResult.buffer,
                audioInfo,
                start,
        ...
        if (videoFrames.length === 0) {
            throw new Error(
                `Failed to decode video frames for segment ${i + 1}. The codec may not be fully supported.`
            )
        }
        ...
        const segmentBlob = await encodeAndMux(
            ...
            audioInfo
                ? { sampleRate: audioInfo.sampleRate, numberOfChannels: audioInfo.numberOfChannels }
                : undefined
        )
