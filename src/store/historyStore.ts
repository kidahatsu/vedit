            ...
            updateClipTrim: (id, trimStart, trimEnd) =>
                set((state) => ({
                    clips: (state?.clips ?? []).map((c) => {
                        ...
                        const validTrimEnd = Math.min(
                            c.duration,
                            Math.max(trimEnd, validTrimStart + 0.05)
                        )
                        return {
                            ...c,
                            trimStart: validTrimStart,
                            trimEnd: validTrimEnd,
                            splitPoints: c.splitPoints.filter(
                                (t) => t > validTrimStart && t < validTrimEnd
                            ),
                        }
                    }),
                })),
...
export function useHistoryActions() {
    ...
    return {
        ...
        get canUndo() {
            return useHistoryStore.temporal.getState().pastStates.length > 0
        },
        get canRedo() {
            return useHistoryStore.temporal.getState().futureStates.length > 0
        },
