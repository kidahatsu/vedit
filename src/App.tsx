...
function App() {
    ...
    const [exportCallback, setExportCallback] = useState<((preset: ExportPreset) => void) | null>(
        null
    )
    ...
    const videoDuration = selectedClip ? selectedClip.trimEnd - selectedClip.trimStart : 0

    const handleOpenExportModal = useCallback(
        (mode: ExportMode, onExport: (preset: ExportPreset) => void) => {
            setExportMode(mode)
            setExportCallback(() => onExport)
            setIsExportModalOpen(true)
            if (mode === 'split' || mode === 'merge') {
                const defaultPreset =
                    EXPORT_PRESETS.find((p) => p.id === 'custom') || EXPORT_PRESETS[0]
